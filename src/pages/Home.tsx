...
export default function Home() {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        await updateDoc(doc(db, 'keys', keyDoc.id), {
          status: 'used',
          usedBy: await getUserIdentifier(),
          usedAt: Date.now(),
          sessionId: sessionId
        });

        // Success!
        setKey(keyData.key || keyData.value);
        ...
        try {
          ...
          if (statsDoc.exists()) {
            ...
            await updateDoc(statsRef, {
              ...
              dailyCount: (statsData.dailyCount || 0) + 1
            });
          } else {
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      if (dailyCount >= 2) {
        throw new Error('Daily limit reached. You can only generate 2 keys per day.');
      }
