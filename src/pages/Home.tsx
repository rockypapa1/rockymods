...
// Helper to get user identifier (more robust than just IP)
const getUserIdentifier = async () => {
  ...
  try {
    ...
    // This handles cases where IP changes slightly but device is the same
    let hash = 0;
  ...
  const handleReturnSession = async (sessionId: string) => {
    ...
    try {
      ...
      if (!sessionDoc.exists()) {
        // Fallback: Check if the session ID matches the one in localStorage
        const localSession = localStorage.getItem('key_session');
        if (localSession && localSession === cleanSessionId) {
          console.warn("Session not found in Firestore yet, retrying...");
          // Wait a bit longer and try again (maybe Firestore is lagging)
      ...
      if (!sessionDoc.exists()) {
        throw new Error('Session not found. Please generate a new link from the main page.');
      }
