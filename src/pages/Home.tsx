...
export default function Home() {
  ...
  const handleReturnSession = async (session: string) => {
    ...
    try {
      ...
      if (!sessionData) {
        ...
        // If already claimed, just show the key
        setLoadingState('Recovering your key...');
        
        // Check if key is already on the session document
        ...
        try {
          ...
          console.error("Recovery from logs failed", recoverErr);
        }

        // If we still don't have it, maybe it was just claimed and not yet written to session
        // Let's try to claim a new one for this session if we really can't find the old one
        // This is the "no bakchodi" approach - just give them a key.
        console.warn("Key not found on claimed session, attempting to assign a new one.");
        // We don't return here, we let it fall through to the claimKeyFromDb call below
      }
      ...
      try {
        ...
        console.warn("Claim failed, retrying...", claimErr);
  ...
  const generateKey = async (duration: string = '1 Day') => {
    ...
    try {
      ...
      // Daily limit check removed as requested by user for "no security"
