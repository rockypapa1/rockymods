...
export default function Home() {
  ...
  const handleReturnSession = async (sessionId: string) => {
    ...
    console.log("Processing return session:", sessionId);
    ...
    try {
      ...
      if (!sessionDoc.exists()) {
        throw new Error('Session not found or expired. Please generate a new link.');
      }
      ...
      if (sessionData.userId !== userId) {
        throw new Error('Security violation: Session mismatch. Please use your own link.');
      }
      ...
      await claimKeyFromDb(sessionId);
    } catch (e: any) {
  ...
  const claimKeyFromDb = async (sessionId: string, retries = 3) => {
    ...
    try {
      ...
      if (isAdBlockerActive) {
        throw new Error('AdBlocker detected! Please disable your AdBlocker to see your key.');
      }

      setLoadingState('Security check: VPN/Proxy...');
      const isVPNActive = await checkVPN();
      if (isVPNActive) {
        throw new Error('VPN/Proxy detected! Please disable your VPN or Proxy to claim your key.');
      }
    } catch (err: any) {
