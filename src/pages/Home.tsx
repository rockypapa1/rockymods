...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const getSessionFromUrl = () => {
      ...
      const sessionMatch = url.match(/[?&/](?:session|session_id|id)[=/]([a-zA-Z0-9_-]{15,})/i);
      if (sessionMatch && sessionMatch[1]) {
        console.log("Found session via Regex:", sessionMatch[1]);
        return sessionMatch[1].trim().replace(/\/$/, '');
      }
      
      // 3. Last resort: check if there's a 20-char alphanumeric string at the end of the URL
      const endMatch = url.match(/[?&/]([a-zA-Z0-9]{20})(?:\/|$)($|\?)/);
  ...
  const handleReturnSession = async (sessionId: string) => {
    ...
    try {
      ...
      // Retry loop (5 attempts with 2s delay)
      if (!sessionDoc.exists()) {
        console.warn("Session not found in Firestore, starting retry loop...");
        for (let i = 0; i < 5; i++) {
          setLoadingState(`Verifying session (Attempt ${i + 1}/5)...`);
