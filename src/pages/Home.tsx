...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const getSessionFromUrl = () => {
      ...
      // 2. Regex fallback (handles mangled URLs like /&session=ID or /?session=ID/)
      // This regex is more aggressive and looks for anything that looks like a Firestore ID after a session param
      const sessionMatch = url.match(/[?&](?:session|session_id|id)=([a-zA-Z0-9_-]{10,})/);
  ...
  const handleReturnSession = async (sessionId: string) => {
    ...
    if (!cleanSessionId || cleanSessionId.length < 10) {
    ...
    try {
      ...
      // Retry loop (3 attempts with 2s delay)
      if (!sessionDoc.exists()) {
        console.warn("Session not found in Firestore, starting retry loop...");
        for (let i = 0; i < 3; i++) {
          setLoadingState(`Verifying session (Attempt ${i + 1}/3)...`);
      ...
      if (!sessionDoc.exists()) {
        ...
        if (localSession && localSession === cleanSessionId) {
          throw new Error('Session was created but not saved correctly. Please try again.');
        }
        throw new Error('Session not found. This can happen if the link was generated too long ago or opened in a different browser.');
      }
