...
export default function Home() {
    ...
    try {
      ...
      if (!sessionData) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            // Method 1: Check keys collection
            const keysRef = collection(db, 'keys');
            const qKeys = query(keysRef, where('sessionId', '==', session), limit(1));
            const keySnap = await getDocs(qKeys);
            if (!keySnap.empty) {
            ...
            // Method 2: Check linkLogs collection
        ...
        throw new Error('This session has already been used and we could not recover your key. Please generate a new one.');
      }
      ...
      try {
        await claimKeyFromDb(session, duration);
      } catch (claimErr: any) {
  ...
  const claimKeyFromDb = async (sessionId: string, duration: string = '1 Day') => {
    ...
    try {
      const keysRef = collection(db, 'keys');
      const qCheck = query(keysRef, where('sessionId', '==', sessionId), limit(1));
  ...
  const generateKey = async (duration: string = '1 Day') => {
    ...
    try {
      ...
      try {
        ...
        if (settingsDoc.exists()) {
          const settingsData = settingsDoc.data();
          // Always use shortener 1 for the first step
