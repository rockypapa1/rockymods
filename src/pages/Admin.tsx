...
export default function Admin() {
  ...
  const deleteAllUsedKeys = async (durationFilter: string = 'All') => {
    let usedKeys = keys.filter(k => k.status === 'used');
    
    if (durationFilter !== 'All') {
      usedKeys = usedKeys.filter(k => k.duration === durationFilter);
    }

    if (usedKeys.length === 0) {
      setError(`No used keys found for ${durationFilter}`);
      return;
    }
    
    setConfirmDialog({
      isOpen: true,
      title: `Delete ${durationFilter} Used Keys`,
      message: `Are you sure you want to delete all ${usedKeys.length} used keys (${durationFilter})? This action cannot be undone.`,
      onConfirm: async () => {
        ...
        try {
          ...
          setBulkDeleteModal({ isOpen: false, status: null });
        } catch (err) {
  ...
  const deleteAllAvailableKeys = async (durationFilter: string = 'All') => {
    let availableKeys = keys.filter(k => k.status === 'available');
    
    if (durationFilter !== 'All') {
      availableKeys = availableKeys.filter(k => k.duration === durationFilter);
    }

    if (availableKeys.length === 0) {
      setError(`No available keys found for ${durationFilter}`);
      return;
    }
    
    setConfirmDialog({
      isOpen: true,
      title: `Delete ${durationFilter} Available Keys`,
      message: `Are you sure you want to delete all ${availableKeys.length} available keys (${durationFilter})? This action cannot be undone.`,
      onConfirm: async () => {
              ...
              <div className="bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden">
                <div className="p-6 border-b border-white/5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setBulkDeleteModal({ isOpen: true, status: 'available' })}
                    ...
                    <button
                      onClick={() => setBulkDeleteModal({ isOpen: true, status: 'used' })}
        ...
        {bulkDeleteModal.isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setBulkDeleteModal({ isOpen: false, status: null })}
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <div className={cn(
                  ...
                  bulkDeleteModal.status === 'available' ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"
                )}>
                  <Trash2 className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white">Bulk Delete {bulkDeleteModal.status === 'available' ? 'Available' : 'Used'} Keys</h3>
                  <p className="text-zinc-400 text-sm">Select which duration of keys to delete.</p>
                </div>
              </div>

              <div className="grid grid-cols-1 gap-3">
                {['1 Day', '2 Days', '3 Days', 'All'].map((duration) => (
                  <button
                    key={duration}
                    onClick={() => {
                      if (bulkDeleteModal.status === 'available') {
                        deleteAllAvailableKeys(duration);
                      } else {
                        deleteAllUsedKeys(duration);
                      }
                    }}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-zinc-500">
                        {keys.filter(k => 
                          k.status === bulkDeleteModal.status && 
                          (duration === 'All' ? true : k.duration === duration)
                        ).length} Keys
                      </span>
                      <Trash2 className="w-4 h-4 text-zinc-600 group-hover:text-red-400 transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
              ...
              <button
                onClick={() => setBulkDeleteModal({ isOpen: false, status: null })}
                className="w-full mt-6 py-3 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-white font-semibold transition-colors"
              >
                Cancel
              </button>
            </motion.div>
