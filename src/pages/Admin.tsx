...
import { Shield, Plus, Trash2, Key, Clock, CheckCircle2, XCircle, LogOut, Link as LinkIcon, Save, Loader2 } from 'lucide-react';
import { clsx, type ClassValue } from 'clsx';
import { twMerge } from 'tailwind-merge';
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, onAuthStateChanged } from 'firebase/auth';
import { collection, query, orderBy, onSnapshot, addDoc, deleteDoc, doc, getDocs, where, setDoc, getDoc, writeBatch } from 'firebase/firestore';
...
export default function Admin() {
  ...
  const [settings, setSettings] = useState<SettingsData>({
    linkShortenerUrl: 'https://xtglinks.com/st',
    linkShortenerApiToken: '0d677cf4096a7a8cfb737c54f7fc8b3a4d043669'
  });
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user && user.email === 'admin@rockymods.com') {
  ...
  const login = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await signInWithEmailAndPassword(auth, 'admin@rockymods.com', password);
    } catch (err: any) {
      if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential' || err.code === 'auth/invalid-login-credentials') {
        try {
          await createUserWithEmailAndPassword(auth, 'admin@rockymods.com', password);
        } catch (createErr: any) {
  ...
  const addKey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const q = query(collection(db, 'keys'), where('value', '==', newKey.trim()));
  ...
  const saveSettings = async (e: React.FormEvent) => {
    ...
    try {
      await setDoc(doc(db, 'settings', 'linkShortener'), {
        linkShortenerUrl: settings.linkShortenerUrl.trim(),
        linkShortenerApiToken: settings.linkShortenerApiToken.trim()
      });
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#0a0a0a]">
        ...
        >
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-zinc-400 text-sm mt-2">Enter your password to continue</p>
          </div>
      ...
      <div className="max-w-5xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-300 transition-colors w-fit"
          >
            ...
            <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-6">
              ...
              <form onSubmit={saveSettings} className="space-y-4">
                <div>
                  ...
                  <input
                    type="url"
                    value={settings.linkShortenerUrl}
                    onChange={(e) => setSettings({ ...settings, linkShortenerUrl: e.target.value })}
                    placeholder="https://xtglinks.com/st"
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all font-mono text-sm"
                    required
                  />
                  <p className="text-xs text-zinc-500 mt-1">Base URL of the shortener API</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-400 mb-2">API Token</label>
                  <input
                    type="text"
                    value={settings.linkShortenerApiToken}
                    onChange={(e) => setSettings({ ...settings, linkShortenerApiToken: e.target.value })}
            ...
            <div className="bg-zinc-900/50 border border-white/5 rounded-2xl overflow-hidden">
              <div className="p-6 border-b border-white/5 flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={isDeletingUsed || keys.filter(k => k.isUsed).length === 0}
                    ...
                    {keys.length === 0 ? (
                      ...
                      keys.map((k) => (
                        <tr key={k.id} className="hover:bg-white/[0.02] transition-colors">
                          <td className="p-4">
                            <span className="font-mono text-sm text-zinc-300">{k.value}</span>
                          </td>
                          <td className="p-4">
                            {k.isUsed ? (
